...
export default function Users() {
  ...
  async function load() {
    ...
    try {
      ...
      setUsers(res.data.data.filter(u => u.role === 'pm'));
    ...
    <AppLayout title="User Management">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button className="btn-primary" onClick={() => { setModal(true); setForm(BLANK); setFormErr(''); }}>+ Add PM</button>
        </div>

        {loading ? <SkeletonTable /> : users.length === 0 ? (
          <EmptyState icon={UsersIcon} title="No users found" description="Add a PM to get started." action={<button className="btn-primary" onClick={() => setModal(true)}>Add PM</button>} />
        ) : (
          <div className="card overflow-hidden">
            <table className="table-base">
              <thead><tr><th>Name</th><th>Email</th><th>Status</th><th>First login</th><th>Actions</th></tr></thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id}>
      ...
      {/* Add PM modal */}
      <Modal open={modal} onClose={() => setModal(false)} title="Add new PM">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">Min 8 chars, uppercase, lowercase, number. PM will be forced to change on first login.</p>
          </div>
          {formErr && <p className="text-sm text-red-600">{formErr}</p>}
          <div className="flex justify-end gap-3">
            <button type="button" className="btn btn-ghost" onClick={() => setModal(false)}>Cancel</button>
            <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Adding…' : 'Add PM'}</button>
          </div>
        </form>
      </Modal>
